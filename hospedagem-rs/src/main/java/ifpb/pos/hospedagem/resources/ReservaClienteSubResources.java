...
public class ReservaClienteSubResources {
    ...
    public Response addClienteEmReserva(
        ...
        if (reserva != null && resposta.getStatus() != Status.NO_CONTENT.getStatusCode()) {
            
            Link clienteLink = resposta.readEntity(Link.class);
            
            System.out.println("SETANDO CLIENTE");
            reserva.setIdCliente(new Long(idCliente));
            ReservaHotel reservaResult = reservaService.atualizar(reserva);
            
            System.out.println("RESERVA SETADA: " + reservaResult);
            
            LinkedReservaHotel reservaLinked = new LinkedReservaHotel(reservaResult, info);
